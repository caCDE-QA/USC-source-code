...
from HQMFv2SymbolTable import ValueSetRhs, CdRhs, ValueAnyNonNullRhs, IvlPqRhs, MyEncoder
import sys
import sqlparse
...
#                sel = sel.select()
    ...
    def apply_set_function(self, code, children, correlate=True):
        ...
        sel = select(table.columns + [self.columns[QDMConstants.ROW_NUMBER]])
        if correlate:
    ...
    def add_temporal_references(self, sel, correlate=True):
        ...
        for t in self.dc.get_temporally_related():
            ...
            if ref.get_id_extension() == 'measureperiod':
                ...
                # Sometimes overkill, but not always
                sel = sel.where(self.get_column(QDMConstants.PATIENT_ID_COL) == selectable.get_column(QDMConstants.PATIENT_ID_COL))
...
class ExtendedSpecificOccurrence(ExtendedDataCriterion):
    def __init__(self, dc, seqno, raw_symbol_table, max_identifier_length=64, max_column_length=32):
        ...
        if dc.is_specific_occurrence_target():
            ...
            if data_type == None:
                data_type="grouper"
            self.so_table_name = 'so_target_' + str(dc.dc_index) + '_' + data_type
        else:
